  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        console.log("Headers enviados:", headers); // <-- Verifica que sí haya token/autenticación
        ...
        console.log("Respuesta de fetch:", res); // <-- Verifica status de respuesta

        if (res.ok) {
          const data: Product[] = await res.json();
          console.log("Productos obtenidos:", data); // <-- Aquí ves lo que llega del backend
          setProducts(data);
        } else {
          console.error("Error al obtener productos. Código:", res.status);
        }
      } catch (error) {
        console.error("Error en fetchProducts:", error); // <-- Captura errores de red
      }
    };
